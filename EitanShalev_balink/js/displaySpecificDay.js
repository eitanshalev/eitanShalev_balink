(function () {
    ...
    var last_click = "Select a country";
    var current_day ="";
    //------------------------------------------------------------------------------------------------------------------
    ...
    document.addEventListener('DOMContentLoaded', function () {
         console.log("im in!");
        var queryString = location.search.substring(1); //The variable queryString now has the value "cityname|date".

        var a = queryString.split("|"); //To break these out into individual variables use split:
        ...
        {
            var replaced_title = a[0].replace("%20", " ");
            current_day = a[1];
    ...
    function showcity_day()
    {
       // console.log("im display the corrent day! :)")
        ...
        var CS = document.getElementById("coutriesSelect");
        var selectedCountry = CS.options[CS.selectedIndex].innerHTML;
        var selectedIdCountry = CS.value;
    ...
    function display_current_day_with_country(selectedIdCountry)
    {
        ...
        fetch("https://www.metaweather.com/api/location/" + selectedIdCountry +"/"+ date[0]+"/"+ date[1] + "/" + date[2]+"/",
            ...
            .then(function (response)
            {
                ...
                html += "<tr>";
                html += "<td>";
            //    html +="<i class='fa fa-long-arrow-up ";
            //    html += arrow_img[0];
          //      html += "></i>";
            ...
            // in order to see the catch case, modify fetch to a non existent URL
            // if you want a quiet error just do:
            // console.log('Request failed', error);
            // if you want the user to know about the error:

            //   a = "<div class=\"alert alert-danger font-weight-bold\">\ Weather forecast service is not available right now, please try again later\ </div>"
            //   a += "<img src=\"images/error.gif\" alt=\"error..\">"
            //    document.querySelector("#data").innerHTML = a;
            //    document.getElementById("loadind").hidden = true;
